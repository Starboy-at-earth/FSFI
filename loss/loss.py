...
class ImgWtLossSoftNLL(nn.Module):
    ...
    def __init__(self, classes, ignore_index=255, weights=None, upper_bound=1.0,
        ...
        self.upper_bound = upper_bound
        self.norm = norm
        self.batch_weights = False #cfg.BATCH_WEIGHTING
        self.fp16 = False
        self.ohem = ohem
    ...
    def custom_nll(self, inputs, target, class_weights, border_weights, mask):
        ...
        REDUCE_BORDER_EPOCH = -1
        EPOCH = 0
        REDUCE_BORDER_EPOCH = -1
        if (REDUCE_BORDER_EPOCH != -1 and EPOCH > REDUCE_BORDER_EPOCH):
            if self.ohem:
    ...
    def forward(self, inputs, target):
        ...
        if (REDUCE_BORDER_EPOCH != -1 and EPOCH > REDUCE_BORDER_EPOCH) and self.ohem:
...
#         #segmask = seg_bord_mask
#         losses = {}

#         losses['seg_loss'] = self.seg_weight * F.binary_cross_entropy_with_logits(input=seg_in, target=seg_bord_mask, reduction='mean')
#         losses['seg_body'] = self.seg_body_weight *  F.binary_cross_entropy_with_logits(input=seg_body_in, target=seg_bord_mask, reduction='mean')
#         losses['edge_loss'] = self.edge_weight * F.binary_cross_entropy(input=edge_in, target=edgemask, reduction='mean')
#         #losses['edge_ohem_loss'] = self.att_weight * self.edge_attention(seg_in, seg_bord_mask, edge_in)
